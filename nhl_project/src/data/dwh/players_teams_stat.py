...
def sat_teams_core(**kwargs):
    ...
    increment = spark.read.parquet(OPERATIONAL_PATH + f"teams_stat")\
        ...
        .select(F.col("team_id"),
                ...
                F.col("_source"))\
        .withColumn("_data_hash", F.sha1(F.concat_ws("_", F.col("team_name"),
                                                    F.col("team_common_name"),
                                                    F.col("conference_abbrev"),
                                                    F.col("conference_name"),
                                                    F.col("division_abbrev"),
    ...
    scd_window = Window.partitionBy("team_id").orderBy(F.col("effective_from").asc(), F.col("is_change").asc())
...
def pit_teams(**kwargs):
    ...
    result = data_versions.withColumn("sat_teams_name_version", F.last("sat_teams_name_version", ignorenulls=True).over(fill)) \
